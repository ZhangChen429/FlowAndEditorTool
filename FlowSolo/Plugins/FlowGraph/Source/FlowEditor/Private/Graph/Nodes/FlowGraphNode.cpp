...
void UFlowGraphNode::DestroyNode()
{
	...
	// Unregister the FlowNode from the FlowAsset before destroying
	// This ensures the node is properly removed from the FlowAsset's internal maps
	// regardless of which editor (SGraphEditor or SFlowGraphEditor) is used
	if (const UFlowNode* FlowNode = Cast<UFlowNode>(GetFlowNodeBase()))
	{
