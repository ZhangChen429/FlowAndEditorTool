...
void UFlowGraphSchema::GetGraphContextActions(FGraphContextMenuBuilder& ContextMenuBuilder) const
{
	...
	// Safely check for paste capability - GetFlowGraphEditor may return nullptr in some contexts
	const TSharedPtr<SFlowGraphEditor> FlowGraphEditor = FFlowGraphUtils::GetFlowGraphEditor(ContextMenuBuilder.CurrentGraph);
...
void UFlowGraphSchema::GetCommentAction(FGraphActionMenuBuilder& ActionMenuBuilder, const UEdGraph* CurrentGraph /*= nullptr*/)
{
	if (!ActionMenuBuilder.FromPin)
	{
		...
		if (CurrentGraph)
		{
			const TSharedPtr<SFlowGraphEditor> FlowGraphEditor = FFlowGraphUtils::GetFlowGraphEditor(CurrentGraph);
			bIsManyNodesSelected = FlowGraphEditor.IsValid() && FlowGraphEditor->GetNumberOfSelectedNodes() > 0;
		}
