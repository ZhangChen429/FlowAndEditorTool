...
def remap_param_type(param_name):
    """Versioning of blender param types"""
    param_remap = {
        ...
        "NodeSocketImage": "TEXTURE",
        "NodeSocketRotation": "ROTATION",
    }

    return param_remap.get(param_name, param_name)
...
def get_geometry_types():
    geometry_types_path = path.abspath(
        path.dirname(path.realpath(__file__)) + "\\GeometryTypes"
    )
    geometry_types = []
